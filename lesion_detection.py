...
#cnt = contours[0]
print(f"total length of contour found={len(contours)}")
cx= xcentro
cy = ycentro
print(f"center of image= {cx, cy}")
success = False
...
cv2.imwrite(save_path_not_cropped, img2)
#cv2.waitKey(0)
...
cv2.imwrite(save_path_cropped, ROI2)
#cv2.imshow("canny", canny)
...
cv2.imwrite(save_path_lesion, img)
